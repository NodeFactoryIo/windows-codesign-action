...
echo "--> signing binary"
/usr/local/bin/osslsigncode sign -pkcs12 /certs/bundle.pfx -pass ${INPUT_PASSWORD} -n ${INPUT_APPNAME} -i ${INPUT_DOMAIN} -t "http://timestamp.digicert.com" -in ${INPUT_BINARYPATH} -out /signedbinary
